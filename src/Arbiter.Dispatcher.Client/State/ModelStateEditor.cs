    ...
    where TUpdateModel : class, new()
{
    ...
    /// <summary>
    /// Creates a new read model instance with the specified identifier and sets it as the current model for editing.
    /// </summary>
    /// <param name="id">The identifier to assign to the new model</param>
    /// <remarks>
    /// <para>
    /// This overload supports upsert scenarios where the caller supplies the identifier of a record that does not
    /// exist yet, so a subsequent <see cref="Save"/> creates the record with that identifier rather than one
    ...
    /// Thrown if the read model type does not have an accessible parameterless constructor or if the mapping fails.
    /// </exception>
    public virtual void New(TKey id)
    {
        ArgumentNullException.ThrowIfNull(id);

        SetModel(new TReadModel { Id = id });
        NotifyStateChanged();
    }

    /// <summary>
    /// Accepts the current state of the model as unmodified by recalculating the <see cref="EditHash"/>.
    /// </summary>
    /// <remarks>
    /// <para>
    /// After calling this method <see cref="IsDirty"/> returns <c>false</c> until the model is changed again.
    /// The model is not sent to the data service; only the change tracking baseline is updated.
    /// </para>
    /// <para>
    /// This is intended for callers that modify the model programmatically and do not want those changes
    /// treated as user edits, for example when applying default values to a newly created model.
    /// </para>
    /// </remarks>
    public void Accept()
    {
        EditHash = Model?.GetHashCode() ?? 0;
        NotifyStateChanged();
    }
    ...
    public async ValueTask Load(TKey id, bool force = false, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var readModel = await DataService
                .Get<TKey, TReadModel>(id, cancellationToken: cancellationToken)
    ...
    public async ValueTask LoadKey(Guid key, bool force = false, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var readModel = await DataService
                .GetKey<TReadModel>(key, cancellationToken: cancellationToken)
                .ConfigureAwait(false);
    ...
    /// <remarks>
    /// <para>
    /// If <see cref="ModelStateManager{TModel}.Model"/> is <c>null</c>, the method returns without performing any operation.
    /// </para>
    /// <para>
    ...
    public async ValueTask Save(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var readModel = await DataService
                .Save<TKey, TUpdateModel, TReadModel>(key!, Model, cancellationToken)
                .ConfigureAwait(false);
    ...
    /// </code>
    /// </example>
    public async ValueTask Delete(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // get key from original
            await DataService
                .Delete<TKey, TReadModel>(Original.Id, cancellationToken)
                .ConfigureAwait(false);
    ...
    /// <summary>
    /// Discards any unsaved changes by resetting the update model back to the state of <see cref="Original"/>.
    /// </summary>
    /// <param name="cancellationToken">
    /// The token to monitor for cancellation requests. The operation completes synchronously, so this token is unused
    /// and is accepted only to match the signature of the other state operations.
    /// </param>
    /// <returns>A completed <see cref="ValueTask"/></returns>
    /// <remarks>
    /// <para>
    /// If <see cref="Original"/> is <c>null</c> there is nothing to revert to and the method returns without
    /// performing any operation.
    /// </para>
    /// <para>
    /// The original read model is re-mapped to a new update model and <see cref="EditHash"/> is recalculated, so
    /// <see cref="IsDirty"/> returns <c>false</c> after this method completes. Any references previously held to
    /// <see cref="ModelStateManager{TModel}.Model"/> are no longer current.
    /// </para>
    /// </remarks>
    /// <exception cref="InvalidOperationException">
    ...
    /// <example>
    /// <code>
    /// // Discard the user's edits and restore the loaded values
    /// if (stateEdit.IsDirty)
    ///     await stateEdit.Cancel();
    /// </code>
    /// </example>
    public ValueTask Cancel(CancellationToken cancellationToken = default)
    {
        if (Original is null)
