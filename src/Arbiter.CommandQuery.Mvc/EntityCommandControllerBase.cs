    ...
    : EntityQueryControllerBase<TKey, TListModel, TReadModel>
{
    ...
    [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(Models.ProblemDetails))]
    ...
    /// <param name="cancellationToken">The cancellation token for the request.</param>
    /// <returns>The read model for the patched entity.</returns>
    protected virtual async Task<TReadModel?> PatchCommand(TKey id, IReadOnlyList<JsonPatchOperation> jsonPatch, CancellationToken cancellationToken = default)
    {
